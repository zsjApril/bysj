...
//        分配变量解析模板   pe_packages=>$pe_package 将参数分配进去
    ...
    public function edit($id)
    {
        ...
        $pe_ques = Pe_questionnaire::findOrFail($id);
        $items = Pe_item::all();
        $pOptions = DB::table('pe_option')->where('question_id', $id)->get();
        //展现用户信息
        return view('pe_questionnaire.edit', ['pe_questionnaires' => $pe_ques,
            'items' => $items, 'pOptions' => $pOptions]);
    }

    public function update(Request $request)
    {
        //  读取信息
        /** @var Pe_questionnaire $pe_ques */
        ...
        if ($pe_ques->save()) {
            ...
            $option_items = $request->get('option_items');

            $pe_ques->removeOptions();
            $pe_ques->addOptions($options, $option_items);
    ...
    public function add()
    {
        ...
        return view('pe_questionnaire.add', compact('options', 'items'));
    }
